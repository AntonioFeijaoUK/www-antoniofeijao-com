---
date: 2025-02-11
last_modified_at: 2025-02-11

title: "AI generated - 2025-02-11 - Cybersecurity News Summary"

#layout: splash
excerpt_separator: <!--more-->
#permalink: /plugins/

categories:
  - ai-generated
  - cybernews
tags:
  - cybernewssummary2025
  - cybernewssummary2025-02-11
  - summary
---

## Key Stories - 2025-02-11 - Cybersecurity News Summary

1. [**Apple fixes iPhone and iPad bug exploited in ‘extremely sophisticated attacks’**](https://securityaffairs.com/174066/hacking/apple-fixes-iphone-and-ipad-bug-exploited-in-extremely-sophisticated-attacks.html):
Apple has released iOS and iPadOS updates to address a critical zero-day vulnerability (CVE-2025-24200), which has been exploited in sophisticated targeted attacks. The vulnerability could allow an attacker to disable USB Restricted Mode on a locked device.

2. [**Operation Phobos Aetor: Police dismantled 8Base ransomware gang**](https://securityaffairs.com/174078/cyber-crime/police-dismantled-8base-ransomware-gang.html):
Authorities conducted an international operation, codenamed Operation Phobos Aetor, leading to the dismantling of the 8Base ransomware gang. The dark web data leak and negotiation sites used by the gang were taken offline.

3. [**UK Gov demands backdoor to access Apple iCloud backups worldwide**](https://securityaffairs.com/174032/laws-and-regulations/uk-gov-demands-backdoor-apple-icloud-backups.html):
The UK government is demanding that Apple create an iCloud backdoor via a Technical Capability Notice, which raises privacy and security concerns.

## Overview

The recent news articles indicate an ongoing trend in cyberattacks becoming increasingly sophisticated and targeted, exemplified by the exploit of Apple's vulnerability in 'extremely sophisticated attacks'.

The successful dismantling of the 8Base ransomware gang showcases the effectiveness of international law enforcement operations in combating organised cybercrime.

However, the government's request to Apple for a backdoor is a reminder that the balance between state security interests and individual privacy concerns remain a contentious issue in the cybersecurity landscape.

---

Happy learning,

[Antonio Feijao UK](https://www.antoniofeijao.com/)
